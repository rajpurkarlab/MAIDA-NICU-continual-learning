"""Fine-tuning support: validation metrics and data loader construction.

This module provides the evaluation and data-plumbing helpers used during
fine-tuning. ``calculate_loss_and_error`` aggregates per-image model output into
mean classification/regression losses plus, per landmark category (tip/carina),
the localization error and recall; here "error" is the Euclidean pixel distance
between the predicted landmark and the ground-truth bbox center, and "recall"
flags whether a prediction was emitted for an annotated landmark. ``validate``
runs the model over a dataloader without gradients, computes these metrics,
optionally logs them to wandb, and returns the summed validation loss used for
epoch selection / early stopping. ``get_data_loader`` assembles train/validation
loaders from the public (MIMIC + RANZCR) splits and/or the hospital dataset with
the target hospital held out.
"""
...
def calculate_loss_and_error(
    ...
    For each image the model output carries classification and regression losses,
    plus a predicted point per landmark class. For every class in ``COCO_LABELS``
    that has a matching ground-truth annotation, the localization error is the
    Euclidean pixel distance between the predicted point and the ground-truth bbox
    center, and recall is recorded as 1 (prediction present for an annotated
    landmark) or 0 (no prediction). Empty metric lists default to a mean of 1.

    Args:
        predictions_dict: Mapping of image_id -> per-class prediction dict.
        annotation_loader: Supplies COCO annotations for ground-truth lookup.

    Returns:
        Dict of mean values keyed by classification/regression loss and by
        per-category error/recall metric names.
    """
    loss_err = {
        CLASSIFICATION_LOSS: [],
        REGRESSION_LOSS: [],
...
def validate(
    ...
    """Run the model over a dataloader and return the summed validation loss.

    Sets the model to eval mode and predicts over all batches without gradients,
    accumulating per-image predictions. These are reduced via
    ``calculate_loss_and_error``; classification and regression losses are summed
    into the returned loss, and (unless ``WANDB_OFF``) all metrics are logged to
    wandb at the given ``val_epoch_step``.

    Args:
        carinaNet_model: The model to evaluate.
        dataloader: Validation/test dataloader.
        annotation_loader: Supplies COCO annotations for metric computation.
        val_epoch_step: Step index used for wandb logging.

    Returns:
        The summed classification + regression validation loss.
    """
    predictions_dict = {}
