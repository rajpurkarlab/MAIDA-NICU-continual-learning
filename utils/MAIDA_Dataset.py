"""PyTorch Dataset for MAIDA NICU chest radiographs.

Loads preprocessed 640x640 images by image id, either from a metadata table
(file paths resolved via the data directory) or by copying an existing dataset.
``reset_image_meta`` re-points the dataset at a subset of images without
reloading paths, which is used to build the cross-validation folds.
"""
