#!/usr/bin/env python3
"""
Convert raw annotation-platform exports into COCO format with stratified splits.

This is the first preprocessing step. It reads the raw annotation exports, resolves
each annotation to a hospital and image file, and writes per-hospital and aggregate
COCO JSON files that the downstream 640x640 preprocessing (preprocess_to_640x640.py)
consumes.

Key steps and choices:
  - Annotations with PROBLEM status are filtered out; only valid entries are kept.
  - Hospital names are normalized to a consistent hyphenated convention, with special
    handling for hospitals whose image filenames differ from their annotation names
    (Essen, Chiang Mai, Osaka).
  - Bounding boxes are stored as both normalized coordinates and absolute coordinates
    in the original image space.
  - Each hospital is split into train (50 images) and test sets using a stratified
    split that preserves the ETT placement distribution, with a fixed random seed for
    reproducibility.
"""
...
# Paths — set these to your local data locations before running.
LATEST_ANNOTATIONS_DIR = "path/to/latest_annotations"   # raw annotation-platform exports
IMAGES_DIR = "path/to/images/hospitals"                 # original-resolution images
MAPPINGS_FILE = "path/to/mappings.csv"                   # new-name -> original-name mapping
OUTPUT_DIR = "path/to/output/annotations"               # where COCO JSONs are written
EXISTING_ANNOTATIONS_DIR = "path/to/existing/annotations"  # prior COCO annotations, if any
