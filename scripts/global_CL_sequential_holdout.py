#!/usr/bin/env python3
"""
Leave-one-out holdout evaluation for continual learning.

Holds out one hospital, runs continual learning over all remaining hospitals,
then evaluates the final trained model on the held-out hospital's test set to
measure generalization to an unseen site. The trained model is used directly
in memory for the holdout inference (no checkpoint reload).
"""
...
# continual_learning is imported as a sibling module in scripts/ (run from the repository root; see README).
...
def main():
    ...
    try:
        ...
        hospital_dir = config.get('output_path', 'outputs/holdout')
