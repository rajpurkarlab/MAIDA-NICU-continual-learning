"""
Core continual learning implementation.

Passes a single CarinaNet model sequentially through a list of hospitals. Each
hospital fine-tunes the model on its local training samples, then the updated
weights are carried forward to the next hospital. This module provides:

  - process_hospital_sequentially(): the core loop shared by every CL experiment
    (standard CL, holdout, single-hospital fine-tuning).
  - main(): entry point for standard CL runs over all hospitals, with multiple
    random orderings (simulations).
"""
...
def main(config):
    ...
    for i in range(config[NUM_SIM]):
        ...
        # Clean up after a simulation: reload data and annotation loaders in case
        # they were modified in place during training.
        data_annos_loaders = get_hospital_data_annos_loaders(config)
