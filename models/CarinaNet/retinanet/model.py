...
from models.CarinaNet.retinanet.augmentation import MedicalImageTransforms
from retinanet.utils import BasicBlock, Bottleneck, BBoxTransform, ClipBoxes
from retinanet.anchors import Anchors
from retinanet import losses
from utils.constants import AUGMENT_DATA, CUDA_AVAILABLE
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        if self.training:
            ...
            if AUGMENT_DATA:
                img_batch, annotations = data_augmentation(img_batch, annotations)
            self.freeze_bn()
        else:
            if type(inputs) == list:
                img_batch, annotations = inputs
        ...
        if self.training:
            ...
            for i in range(classification.shape[2]):
                ...
                scores_over_thresh = scores > THRESHOLD_SCORE
