...
Utilities for converting landmark pixel coordinates into physical ETT-carina
distances (cm). Pixel distances are calibrated to centimeters using the
annotated ETT width as a patient-specific reference, with a weight- or
gestational-age-based fallback when the measured width is unavailable.

Data inputs (the underlying patient data is not distributed with this
repository; the descriptions below document the expected formats so the pipeline
is readable without the data):

  - mappings.csv: the bridge between predictions and demographics. Each row maps
    a de-identified image id ("new_name" -- the random id used in image
    filenames and prediction CSVs) to that image's per-hospital patient
    identifier ("original_name", e.g. "p54-study-...") and its institution
    ("institution"). Predictions are keyed by the random id, while demographics
    are keyed by patient number, so this file is what links the two.
  - data/demographics/<Hospital>_nicu_settings.csv: per-hospital patient
    demographics (birth weight in grams, gestational age in weeks) used to
    select the ETT width when calibrating the pixel-to-cm conversion.
  - data/clinical_annotations.json: expert ETT/carina annotations, including the
    measured ETT width in pixels that serves as the calibration reference.
  - data/annotations/preprocessed_640x640/: COCO-format bounding boxes and image
    metadata (see data/example_annotations.json for the exact schema).
...
# Paths to data within the repository (relative to repo root). The data itself
# is not shipped; see the module docstring and data/README.md for the formats.
DATA_DIR = REPO_ROOT / "data"
ANNOTATIONS_DIR = DATA_DIR / "annotations" / "preprocessed_640x640"   # COCO bbox + image metadata
DEMOGRAPHICS_DIR = DATA_DIR / "demographics"                          # per-hospital weight / gestational-age CSVs
# mappings.csv links de-identified prediction ids ("new_name") to each hospital's
# patient identifier ("original_name") and institution ("institution"), so a
# prediction can be matched back to the correct demographics row.
MAPPINGS_CSV = DATA_DIR / "mappings.csv"
CLINICAL_ANNOTATIONS_JSON = DATA_DIR / "clinical_annotations.json"    # expert ETT/carina annotations + measured ETT pixel width
...
def load_all_clinical_annotations() -> Tuple[Dict, Dict]:
    ...
    if not latest_dir.exists():
        ...
        print(f"Note: clinical annotations are normally pre-computed in {CLINICAL_ANNOTATIONS_JSON}")
...
def get_patient_weight_standardized(random_id: str, mappings: pd.DataFrame,
    ...
    Args:
        ...
        mappings: the mappings.csv DataFrame (columns: new_name, original_name, institution)
