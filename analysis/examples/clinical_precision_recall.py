#!/usr/bin/env python3
"""
Clinical triage analysis: precision@K, recall@K, and false-positive rate for
flagging abnormal ETT placements, comparing continual learning (CL) against
single-hospital fine-tuning (FT) and direct prediction.

For each hospital, radiographs are ranked by predicted ETT-carina distance
(shorter distance = higher concern for deep placement) and the top-K are
scored against the clinician-assigned placement labels. Predicted pixel
distances are converted to centimeters using the annotated ETT width as an
...
Direct as a single deterministic run. CL vs FT uses a two-sample t-test; CL
vs Direct uses a one-sample t-test (Direct is a single point estimate).
Metrics are reported for K = 5 and K = 15.
"""
