#!/usr/bin/env python3
"""
Localization-error comparison across the three adaptation strategies:
continual learning (CL), single-hospital fine-tuning (FT), and direct
prediction with the unadapted adult model.

For each hospital it computes the median ETT-tip and carina localization
errors (cm), then reports per-method means with 95% confidence intervals and
renders the CL-vs-FT and three-method comparison figures.

Per-method variability follows the study design: CL across random hospital
orderings (10 simulations), FT across random seeds (10 simulations), and
Direct as a single deterministic run. Significance uses a two-sample t-test
for CL vs FT and a one-sample t-test for CL vs Direct (Direct is a single
point estimate).
"""
...
def get_hospital_display_name(normalized_name: str) -> str:
    """Map a normalized hospital key to its formal display name, tolerating a trailing dash."""
...
def main():
    """Run the localization-error comparison and write the tables and figures."""

    num_sims = 10

    print("="*80)
    print("LOCALIZATION ERROR COMPARISON: CL vs FT vs DIRECT")
    print("="*80)
